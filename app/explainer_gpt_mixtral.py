...
def call_llm(prompt: str, max_tokens: int = 120) -> str:
    response = groq_client.chat.completions.create(
        model=LLM_MODEL,
        messages=[
            {"role": "system", "content": "You are a financial analyst generating formal, decision-based explanations for loan applications."},
...
def explain_with_mixtral(top_shap_feats: list[FeatureImpact], top_lime_feats: list[dict]):
    ...
    if narrative is None or not isinstance(narrative, str) or not narrative.strip():
        narrative = "Explanation temporarily unavailable due to Mixtral API error."
